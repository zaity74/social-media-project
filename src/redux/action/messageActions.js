...
export const fetchConversations = (userId) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.get(`https://socialmedy.netlify.app/conversations/${userId}`);
...
export const fetchMessages = (conversationId) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.get(`https://socialmedy.netlify.app/${conversationId}`);
...
export const sendMessage = (message) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.post("https://socialmedy.netlify.app/send", modifiedMessage, {
      headers: {
...
export const createConversation = (senderId, receiverId) => async (dispatch) => {
  ...
  try {
    const { data } = await axios.post(`https://socialmedy.netlify.app/conversations`, {
      senderId,
      receiverId,
    });
...
export const deleteConversation = (id) => async (dispatch) => {
  ...
  try {
    await axios.delete(`https://socialmedy.netlify.app/conversations/${id}`);
