...
export const getNotifications = (userId) => async (dispatch) => {
    try {
        ...
        const response = await axios.get(`https://socialmedy.netlify.app/notification/${userId}`);
...
export const clearNotifications = (userId) => async (dispatch) => {
    try {
      ...
      await axios.delete(`https://socialmedy.netlify.app/notification/delete/${userId}`);
...
export const getUnreadNotificationCount = (userId) => async (dispatch) => {
  try {
    const { data } = await axios.get(`https://socialmedy.netlify.app/unread-count/${userId}`);
    dispatch({
