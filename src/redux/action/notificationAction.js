...
export const getNotifications = (userId) => async (dispatch) => {
    try {
        ...
        const response = await axios.get(`http://localhost:8081/notification/${userId}`);
...
export const clearNotifications = (userId) => async (dispatch) => {
    try {
      ...
      await axios.delete(`http://localhost:8081/notification/delete/${userId}`);
...
export const getUnreadNotificationCount = (userId) => async (dispatch) => {
  try {
    const { data } = await axios.get(`http://localhost:8081/unread-count/${userId}`);
    dispatch({
