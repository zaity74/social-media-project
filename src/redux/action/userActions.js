...
export const updateUser = (userId, userData) => async (dispatch) => {
  try {
    ...
    const response = await axios.put(`http://localhost:8081/user/${userId}`, userData, config);
...
export const followUser = (userId, targetUserId) => async (dispatch) => {
  try {
    ...
    const response = await axios.put(
      `http://localhost:8081/user/follow/${targetUserId}`,
      { userId },
      config
    );
...
export const unfollowUser = (userId, targetUserId) => async (dispatch) => {
  try {
      ...
      const response = await axios.put(
          `http://localhost:8081/user/unfollow/${targetUserId}`,
